...
export default function ErrorBoundary({
  ...
  const message = isExpected ? error.message : "Something went wrong while loading this page.";

  return (
    <div className="flex flex-1 flex-col items-center justify-center px-6 py-24 text-center">
      <AlertTriangle className="size-10 text-amber-400" aria-hidden="true" />
      <h2 className="mt-4 text-xl font-semibold text-zinc-900">
        {isExpected ? "Unable to load content" : "Something went wrong"}
      </h2>
      <p className="mt-2 max-w-sm text-sm text-zinc-500">{message}</p>
      {!isExpected ? (
        <p className="mt-1 text-xs text-zinc-400">Please try again in a moment.</p>
      ) : null}
      <button
        type="button"
        onClick={reset}
